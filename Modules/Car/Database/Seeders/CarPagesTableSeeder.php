...
class CarPagesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $content = new Content;
        $content->title = 'My Page - Notiifcations';
        $content->slug = 'my-notifications';
        $content->type = Content::TYPE_PAGE;
        $content->status = Content::STATUS_PUBLISHED;
        $content->visibility = Content::VISIBILITY_AUTH;
        $content->author_id = 1;
        ...
        $file_content = file_get_contents(resource_path('stubs/carNotifications.stub'));
        $file_name = $rootPath . DIRECTORY_SEPARATOR . 'my-notifications' . Content::NAMING_CONVENTION . $content->status . Content::NAMING_CONVENTION . $time;
        $file_ext = 'blade.php';
        $file_path = $file_name . '.' . $file_ext;
